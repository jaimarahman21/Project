item_dict={}
f=open("F:/jaima/restaurant_management.txt","r")
quantities=[]
prices=[]
items=[]
while True:
    ...
    if items !='':
        items.append(item)
    if quant !='':
        quantities.append(int(quant))
    if uprice !='':
        prices.append(int(uprice))
    #quant=int(quant[len(quant)-1])
    #uprice=float(uprice[:len(uprice)-1])
    if quant!='' and uprice!='':
        item_dict[item]=[int(quant),int(uprice)]
f.close()
print(items)
print(quantities)
print(prices)
...
while True:
    ...
    if choice=='1':
        ...
        continue
f=open("F:/jaima/restaurant_management.txt","w")
for x in item_dict:
    f.write(x+"\n")
